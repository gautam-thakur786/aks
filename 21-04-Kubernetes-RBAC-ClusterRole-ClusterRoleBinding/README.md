...
az role assignment create --assignee $AKS_READONLY_GROUP_ID --role "Azure Kubernetes Service Cluster User Role" --scope $AKS_CLUSTER_ID

# Create AKS ReadOnly User in Azure AD
AKS_READONLY_USER_OBJECT_ID=$(az ad user create --display-name "AKS READ1" --user-principal-name aksread1@stacksimplifygmail.onmicrosoft.com --password @AKSDemo123 --query objectId -o tsv)
